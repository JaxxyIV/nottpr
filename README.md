...
### What was the reason for adding builders?

At the moment, the ALTTPR API is very poorly understood and documented,
especially when using the customizer. Builders are meant to be as much of an
expansive documentation of the API as possible. Additional enums are also
included to help further simplify what values are acceptable for a particular
setting.

### Do I have to use builders for seed generation?

No. Object literals can also be passed to the generator methods. The only
difference between using a builder and using an object literal is that default
values are not given to missing properties when using an object literal.

Whether builders or object literals are used is up to developer preference.

Alternatively, you can pass a callback function to the generators. This allows
you to use builders indirectly without importing them:

```js
const seed = await ALTTPR.randomizer((builder) =>
    builder.setAccessibility(Accessibility.Locations)
        .setDungeonItems(Keysanity.Full)
        .setEntrances(Entrances.Crossed)
        .setGoal(Goals.FastGanon)
);
...
### Why do `Seed.patchROM` and `Sprite.fetch` return buffers instead of writing files?

This was a design choice I made to give more power to developers. Some may not
want these methods to write a file to their disk automatically, so the methods
return the data as a buffer and how that data is to be handled is up to whoever
is implementing the module.

You can write a file to the disk using the `fs.writeFile` method.

### Are there plans for compatibility with SahasrahBot?

Not right now. SahasrahBot YAMLs have too many unique features exclusive to them
that I would have to work around in order to make them compatible with nottpr.

I plan to make a mystery weightset parser in the future that will work alongside
nottpr, but will otherwise be a separate module.
